...
| 真船照片识别 | 泛化 100%, 判别 95% | 颜色定位+填平+剪影 few-shot |

### 真船照片识别的完整配方（2026-08-16）

真实照片（红棕船 R=52 vs 蓝海 R=1）在灰度下船海只差 0.03（< 噪声，信息论上分不开），但 R 通道差 51 倍。全链配方：

```
颜色定位(R通道, figure-ground) → 填平内部(binary_fill_holes) → 灰度剪影 → few-shot → 识别
```

四坎逐个攻破：低对比度（centering）、背景（颜色定位+裁剪）、纹理（填平去窗户甲板）、形状风格（剪影 few-shot 替代方块合成船）。

**关键哲学**：颜色不是语义捷径（非"红=船"），是**图形-背景分离**（figure-ground，如视网膜色拮抗细胞）——只用于找边界，识别仍走灰度，不违背灰度-first 哲学。
