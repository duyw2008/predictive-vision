#!/usr/bin/env python3
"""CIFAR-10 验证: 加载 + ColdEye v3 训练 + 对比度不变性."""
import sys, os, time, numpy as np
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))
from train_multiscale import ColdEye, load_cifar10, low_contrast

np.random.seed(42)
print("加载 CIFAR-10 (灰度)...")
t0 = time.time()
X_tr, y_tr, X_te, y_te = load_cifar10(n_train_per_class=2000, n_test_per_class=500)
print(f"  训练 {X_tr.shape} 测试 {X_te.shape} — {time.time()-t0:.0f}s")
print(f"  灰度范围 [{X_tr.min():.2f}, {X_tr.max():.2f}], 标签分布 {np.bincount(y_tr)}")

# ColdEye v3: global 100 + patch 16×16/50 (32×32 图)
model = ColdEye()
print("\n训练 ColdEye v3 (global 100 + patch 16/50)...")
t0 = time.time()
